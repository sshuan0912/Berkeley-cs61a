...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place.is_hive == False:
            if current_place.bees != [] and self.lower_bound <= k <= self.upper_bound:
...
class HungryAnt(Ant):
    ...
    food_cost = 4
    implemented = True
    chewing_turns = 3   # the number of turns that it will take to chew
    def __init__(self, health=1):
        super().__init__(health)
        self.turns_to_chew = 0  # counts the number of turns it has left to chew
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.has_queen is False: # check if an Ant can be constructed
            gamestate.has_queen = True
            return QueenAnt.construct(gamestate)
        else:
            return None
        # END Problem 12
