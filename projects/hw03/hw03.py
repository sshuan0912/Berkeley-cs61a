...
def pingpong(n):
    ...
    # d, i, total as argument
    def helper(d,i,total):
        if i == n:
            return total
        else:
            if num_eights(i) >= 1 or i%8 == 0:
                return helper(-d, i+1, total-d)
            else:
                return helper(d, i+1, total+d)
...
def count_coins(change):
    ...
    if change < 5:
        return 1
    elif change <10 or change >= 5:
    else:
        next_coin = next_smaller_coin(change)
        count_left = count_coins(change - next_coin)
        count_next_coin = count_coins(next_coin)
        return count_left + count_next_coin
