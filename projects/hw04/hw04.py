...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return length(left(m))*total_weight(left_end) == length(right(m))*total_weight(right_end) and balanced(left_end) and balanced(right_end)
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        bs = [  totals_tree(end(a)) for a in [left(m), right(m)] ]
...
def has_path(t, word):
    ...
    if label(t) == word:
        return True
    elif label(t)[0] != word[0]:
        return False
    else:
        paths = [has_path(b,word[1:]) for b in branches(t)]
        for path in paths:
            if path:
                return True
    return False
